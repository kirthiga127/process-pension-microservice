...
import com.pms.pension.process.feignclient.PensionDisbursementFeignClient;
import com.pms.pension.process.feignclient.PensionerDetailFeignClient;
	...
	void testMockMvcNotNull() {
	  ...
	  @Test 
		void testGetprocessingCode() throws Exception {
			...
			mockMvc.perform(post("/api/final/ProcessPension").contentType("application/json").content(jsonPensionerInput).header("Authorization", "Bearer @token@token"))
	  ...
	  @Test
		void testGetResponseInvalidAadharCard() throws Exception {
		 ...
		 when(authorisingClient.authorizeTheRequest("Bearer @token@token")).thenReturn(true);
			...
			String jsonPensionerInput = this.mapToJson(processPensionInput);
			mockMvc.perform(post("/api/final/disbursePension").contentType("application/json").content(jsonPensionerInput).header("Authorization", "Bearer @token@token"))
	  ...
	  @Test 
		void testGetResponse() throws Exception {
			...
			mockMvc.perform(post("/api/final/PensionDetail").contentType("application/json").content(jsonPensionerInput).header("Authorization", "Bearer @token@token"))
			.andExpect(status().isOk());
		}
	  @Test 
	  void testGetPensionDetail() throws Exception {
		...
		void testGetPensionDetailBadRequest() throws Exception {
				...
				pensionerInput.setPensionType("self");
				String jsonPensionerInput = this.mapToJson(pensionerInput);
				mockMvc.perform(post("/api/final/PensionDetail").contentType("application/json").content(jsonPensionerInput)).andExpect(status().isBadRequest());
			}
